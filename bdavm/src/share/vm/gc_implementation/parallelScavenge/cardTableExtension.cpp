...
CardTableExtension::scavenge_bda_contents_parallel(ObjectStartArray * start_array,
                                                   ...
                                                   PSPromotionManager * pm)
{
  assert (c->_start < c_top, "Should not be called if empty");
  oop * sp_top = (oop*)c_top;
...
/**
 * This version of scavenge_contents_parallel includes, instead of a ptr to the MutableSpace
 * which usually is the object space of the old-gen, the starting pointer of the slice that is
 * not part of a container segment. The slice_top is the starting address of the next container
 * segment or the actual top ptr of the object space.
 */
void CardTableExtension::scavenge_nonbda_contents_parallel(ObjectStartArray* start_array,
  ...
  // It is a waste to get here if empty.
  assert(slice_bottom <= slice_top, "Wrong slice bottom and top pointers.");
  oop* sp_top = (oop*)slice_top;
  jbyte* start_card = byte_for(slice_bottom);
  jbyte* end_card   = byte_for(sp_top - 1) + 1;
  oop* last_scanned = NULL; // Prevent scanning objects more than once
  // The width of the stripe ssize*stripe_total must be
  // consistent with the number of stripes so that the complete slice
  ...
  for (jbyte* slice = start_card; slice < end_card; slice += slice_width) {
    ...
    // Update our beginning addr
#ifdef BDA
    HeapWord* first_object = start_array->object_start(slice_start, slice_bottom);
#else
    HeapWord* first_object = start_array->object_start(slice_start);
#endif
    debug_only(oop* first_object_within_slice = (oop*) first_object;)
...
#ifdef BDA
          HeapWord* last_object_in_dirty_region = start_array->object_start(
            addr_for(current_card)-1, slice_bottom);
#else
          HeapWord* last_object_in_dirty_region = start_array->object_start(addr_for(current_card)-1);
#endif
          size_t size_of_last_object = oop(last_object_in_dirty_region)->size();
