...
objArrayOop InstanceKlass::allocate_objArray(int n, int length, TRAPS) {
  ...
  {
      ResourceMark rm(THREAD);
      // Trying to fire the creation of an HashMap$Node array
      if(strstr(h_ak->signature_name(), "HashMap$Node")) {
          if (layout_helper_is_array(layout_helper())) {
              tty->print_cr("Array layout_helper: hsz = %d ; log2(esz) = %d",
                            Klass::layout_helper_header_size(layout_helper()),
                            Klass::layout_helper_element_type(layout_helper()));
          }
      }
  }
